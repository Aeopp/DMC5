...
bool NeroState::m_bPlayRedQueenSound = true;
bool NeroState::m_bPlayOvertureSound = true;
bool NeroState::m_bPlayCbsShortSound = true;
bool NeroState::m_bPlayCbsMiddleSound = true;
bool NeroState::m_bPlayCbsLongSound = true;
bool NeroState::m_bPlayDanteVoice = true;
...
HRESULT NeroState::StateEnter()
{
	...
	m_bPlayCbsShortSound = true;
	m_bPlayCbsMiddleSound = true;
	m_bPlayCbsLongSound = true;
	m_bPlayDanteVoice = true;
	return S_OK;
}
...
void NeroState::Play_Sound(GAMEOBJECTTAG _Tag, const std::string& SoundKey, bool _Overlap)
{
	switch (_Tag)
	{
	case Player:
	...
	case Tag_Cbs_Short:
		...
		break;
	case Tag_Cbs_Middle:
		if (m_bPlayCbsMiddleSound)
		{
			m_bPlayCbsMiddleSound = false;
			SoundSystem::GetInstance()->Play(SoundKey, 0.7f, _Overlap);
		}
		break;
	case Tag_Cbs_Long:
		if (m_bPlayCbsLongSound)
		{
...
HRESULT RunStartLeft::StateEnter()
{
	...
	SoundSystem::GetInstance()->Play("Walk1", 0.6f, true);
...
HRESULT DashLoop::StateEnter()
{
	...
	m_pNero.lock()->ChangeNeroDirection(Nero::Dir_Front);
	m_fGradient = 0.f;
	SoundSystem::GetInstance()->Play("DashStartCloth", 0.4f, true);
...
HRESULT Wind_Pressure_Big::StateUpdate(const float _fDeltaTime)
{
	if (m_pNero.lock()->IsAnimationEnd())
	{
		m_pFSM->ChangeState(NeroFSM::WINDPRESSURE_END);
		return S_OK;
	}
	return S_OK;
}
...
Wind_Pressure_CutScene::Wind_Pressure_CutScene(FSMBase* const _pFSM, const UINT _nIndex, weak_ptr<Nero> _pNero)
	:NeroState(_pFSM, _nIndex, _pNero)
{
}

Wind_Pressure_CutScene::~Wind_Pressure_CutScene()
{
}

Wind_Pressure_CutScene* Wind_Pressure_CutScene::Create(FSMBase* const _pFSM, const UINT _nIndex, weak_ptr<Nero> _pNero)
{
	return new Wind_Pressure_CutScene(_pFSM, _nIndex, _pNero);
}

HRESULT Wind_Pressure_CutScene::StateEnter()
{
	NeroState::StateEnter();
	if (Nero::NeroCom_RedQueen == m_iNeroCurWeaponIndex)
		m_pNero.lock()->Set_Weapon_State(Nero::NeroCom_RedQueen, Nero::WS_Idle);
...
HRESULT Wind_Pressure_CutScene::StateExit()
{
	return S_OK;
}

HRESULT Wind_Pressure_CutScene::StateUpdate(const float _fDeltaTime)
{
	if (m_pNero.lock()->IsAnimationEnd())
	{
...
HRESULT Wire_Pull::StateEnter()
{
	...
	if (Nero::NeroCom_RedQueen != m_iNeroCurWeaponIndex)
		NeroState::SetCbsIdle();
	SoundSystem::GetInstance()->Play("Snatch", 0.8f, true);
	return S_OK;
}
...
HRESULT Wire_Fly_Loop::StateEnter()
{
	...
	SoundSystem::GetInstance()->Play("DanteEff_3", 0.35f, true);
	return S_OK;
}
...
HRESULT BT_Att_ComboD_1::StateEnter()
{
	...
	m_pNero.lock()->CheckAutoRotate();
	NeroState::ActiveTrail(true);
	Play_Sound(TAG_RedQueen, "ComboD1");
	SoundSystem::GetInstance()->RandSoundKeyPlay("DanteAtt", { 1,11 }, 0.7f, false);
	return S_OK;
}
...
HRESULT BT_Att_ComboD_2::StateEnter()
{
	...
	m_pNero.lock()->CheckAutoRotate();
	NeroState::ActiveTrail(true);
	Play_Sound(TAG_RedQueen, "ComboA1,ComboD2");
	SoundSystem::GetInstance()->RandSoundKeyPlay("DanteAtt", { 1,11 }, 0.7f, false);
	return S_OK;
}
...
HRESULT BT_Att_ComboD_3::StateEnter()
{
	...
	m_pNero.lock()->CheckAutoRotate();
	NeroState::ActiveTrail(true);
	Play_Sound(TAG_RedQueen, "ComboD3");
	SoundSystem::GetInstance()->RandSoundKeyPlay("DanteAtt", { 1,11 }, 0.7f, false);
	return S_OK;
}
...
HRESULT BT_Att_ComboD_4::StateEnter()
{
	...
	m_pNero.lock()->CheckAutoRotate();
	NeroState::ActiveTrail(true);
	Play_Sound(TAG_RedQueen, "ComboD4");
	SoundSystem::GetInstance()->RandSoundKeyPlay("DanteAtt", { 1,11 }, 0.7f, false);
	return S_OK;
}
...
HRESULT Cbs_ComboA1::StateEnter()
{
	...
	SoundSystem::GetInstance()->Play("DanteCbsAtt_1", 1.f, true);
	return S_OK;
}
...
HRESULT Cbs_ComboA2::StateEnter()
{
	...
	NeroState::ActiveTrail(true);
	SoundSystem::GetInstance()->Play("CbsShort1", 0.3f, true);
	SoundSystem::GetInstance()->Play("DanteCbsAtt_2", 1.f, true);
	return S_OK;
}
...
HRESULT Cbs_ComboA3::StateEnter()
{
	...
	NeroState::ActiveTrail(true);
	SoundSystem::GetInstance()->Play("CbsShort1", 0.3f, true);
	SoundSystem::GetInstance()->Play("DanteCbsAtt_3", 1.f, true);
	return S_OK;
}
...
HRESULT Cbs_ComboA4::StateEnter()
{
	...
	NeroState::ActiveTrail(true);
	SoundSystem::GetInstance()->Play("CbsShort1", 0.3f, true);
	SoundSystem::GetInstance()->Play("DanteCbsAtt_4", 1.f, true);
	return S_OK;
}
...
HRESULT Cbs_ComboA5::StateUpdate(const float _fDeltaTime)
{
	...
	else if (0.26f <= fCurrAnimationTime)
	{
		...
		if (m_bPlayCbsShortSound)
		{
			...
			SoundSystem::GetInstance()->Play("CbsShort2", 0.3f, true);
			SoundSystem::GetInstance()->RandSoundKeyPlay("JJallang", { 1,5 }, 0.2f, false);
			SoundSystem::GetInstance()->Play("DanteCbsAtt_5", 1.f, true);
		}
	}
	else if (0.14f <= fCurrAnimationTime)
		ActiveColl_Cbs(false, Nero::NeroCom_Cbs_Short);
...
HRESULT Cbs_SKill_IceAge_Start::StateEnter()
{
	...
	SoundSystem::GetInstance()->Play("DanteCbsSkill_1", 1.f, true);
...
HRESULT Middle_Cbs_BiAttack::StateUpdate(const float _fDeltaTime)
{
	...
	else if (0.54f <= fCurrAnimationTime)
	{
		...
		if (m_bPlayRedQueenSound)
		{
			...
			SoundSystem::GetInstance()->RandSoundKeyPlay("DanteAtt", { 1,12 }, 1.f, false);
		}
	}
...
HRESULT Middle_Cbs_BlitzAttack::StateUpdate(const float _fDeltaTime)
{
	...
	if (0.33f <= fCurrAnimationTime && m_bPlayOnce)
	{
		...
		m_pNero.lock()->ChangeWeaponCollSize(0.5f);
		NeroState::ActiveColl_Cbs(true, Nero::NeroCom_Cbs_Middle);
		...
		SoundSystem::GetInstance()->Play("DanteSkill_12", 1.f, false);
	}
...
HRESULT Pole_ComboB4::StateUpdate(const float _fDeltaTime)
{
	...
	else if (0.24f <= fCurrAnimationTime)
	{
		...
		SoundSystem::GetInstance()->Play("DanteSkill_5", 1.f, false);
	}
	else if (0.17f <= fCurrAnimationTime)
		ActiveColl_Cbs(false, Nero::NeroCom_Cbs_Long);
	else if (0.15f <= fCurrAnimationTime)
...
HRESULT Pole_WhirlWind_End::StateUpdate(const float _fDeltaTime)
{
	...
	if (0.25f <= fCurrAnimationTime)
	{
		...
		{
			m_bPlayOnce = false;
			m_pNero.lock()->PlayEffect(Eff_WhirlWind);
			SoundSystem::GetInstance()->Play("DanteSkill_5", 1.f, true);
			m_pNero.lock()->ChangeWeaponCollSize(0.11f);
		}
	}
	else if (0.21f <= fCurrAnimationTime)
	{
		m_pNero.lock()->Set_Weapon_AttType(Nero::NeroCom_Cbs_Long, ATTACKTYPE::Attack_KnocBack);
		ActiveColl_Cbs(true, Nero::NeroCom_Cbs_Long);
		m_pNero.lock()->ChangeWeaponCollSize(0.4f);
		if (m_bPlayOvertureSound)
		{
			m_bPlayOvertureSound = false;
			SoundSystem::GetInstance()->Play("LongComboB_3", 0.4f, false);
...
HRESULT Hr_Ex_Air_Roll_Start::StateEnter()
{
	...
	//나중에 생길 불꽃이펙트 여기서
	SoundSystem::GetInstance()->Play("Hr_Air_ex", 0.2f, true);
	SoundSystem::GetInstance()->Play("RedQueen9", 0.15f, true);
	return S_OK;
}
