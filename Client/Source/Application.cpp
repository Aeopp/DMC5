...
void Application::ElectricParticlePoolLoad()
{
	...
	if (_TargetMesh->m_spVertexLocations)
	{
		for (auto& _ParticleInstance : *ParticlePool)
		{
			...
			{
				Cp0 = StartLocation + Dir * FMath::Random(2500.f, 5000.f);
				Cp1 = Cp0 + Dir * FMath::Random(2500.f, 5000.f);
				End = Cp1 + Dir * FMath::Random(2500.f, 5000.f);
			}
			...
			const float RScale = FMath::Random(0.0033f, 0.008f) * GScale;
